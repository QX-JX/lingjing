...
export type MarkerType = 'pause' | 'repeat' | 'number' | 'polyphone' | 'speed' | 'reread';
 ...
 * 为选中的文本添加重读标记（强调/重音）
 * @param text 原始文本
 * @param start 选中文本的起始位置
 * @param end 选中文本的结束位置
 * @returns 添加标记后的文本和新的光标位置
 */
export function wrapReread(
  text: string,
  start: number,
  end: number
): { newText: string; newPosition: number } {
  if (start >= end) return { newText: text, newPosition: start };

  const before = text.slice(0, start);
  const selected = text.slice(start, end);
  const after = text.slice(end);

  const marker = `<reread>${selected}</reread>`;
...
export function wrapPolyphone(
  ...
  // 清理选中文本中可能存在的其他标记（简单处理）
  const cleanSelected = selected.replace(/<[^>]+>/g, '');

  const marker = `<polyphone pronunciation="${pronunciation}">${cleanSelected}</polyphone>`;
  const newText = before + marker + after;
  const newPosition = start + marker.length;
...
export function removeAllMarkers(text: string): string {
  const markerTypes: MarkerType[] = ['pause', 'repeat', 'number', 'polyphone', 'speed', 'reread'];
  let result = text;
...
export function validateMarkers(text: string): { isValid: boolean; errors: string[] } {
  ...
  const tagRegex = /<(pause|repeat|number|polyphone|speed|reread)[^>]*>|<\/(pause|repeat|number|polyphone|speed|reread)>/g;
